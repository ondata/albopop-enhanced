          ...
          @codec.decode(item.description) do |event|
            ...
            if ! item.source.nil?
              ...
              if ! item.source.url.nil?
                event["source"]["link"] = item.source.url
              end
              event["source"]["title"] = item.source.content
            end
            if ! item.categories.nil?
              event["tags"] = []
              item.categories.each do |cat|
                event["tags"] << cat.content
              end
            end
            decorate(event)
            queue << event
        ...
        when 'atom'
          ...
          @codec.decode(content) do |event|
            ...
            if ! item.categories.nil?
              event["tags"] = []
              item.categories.each do |cat|
                if ! cat.term.nil? || ! cat.label.nil?
                  event["tags"] << cat.term || cat.label
                end
              end
            end
            decorate(event)
